...
{
    /// <summary>
    /// Classe para armazenar um comando de seleção 'if'.
    /// </summary>
    public class SeleçãoIf : ITipo
    {
        ...
        public SeleçãoIf(List<SemanticoItem> elementos, List<ITipo> listaTiposEscopoMaior)
        {
            if (elementos == null)
                throw new ArgumentNullException(nameof(elementos));
            if (elementos.Count < 8)
                throw new Exception("Um tipo 'seleção if' deve receber no mínimo 8 elementos!");
        ...
        /// <summary>
        /// Verifica as variáveis do comando de seleção 'if' e dos comandos dos escopos internos.
        /// </summary>
            ...
            if (this.ListaTiposEscopoMaior != null && this.ListaTiposEscopoMaior.Count > 0)
            {
                ...
                foreach (var item in this.ListaTiposEscopoMaior)
                {
                    ...
                    else if (tipoItem == typeof(Atribuição))  // Se tipo do comando for atribuição, então:
                    {
                        ...
                        // Se a variável que recebe a operação não está na lista de variáveis atribuidas, então adiciona:
                    ...
                    else if (tipoItem == typeof(SeleçãoIf))  // Se tipo do comando for uma seleção 'if', então:
                    {
                        ...
                        // Existirem variáveis declaradas no escopo atual, então anexa à lista do escopo global:
                        if (this.listaVariáveisLocal.Count > 0)
                        {
                            //listaVariáveisEscopo.AddRange(this.listaVariáveisLocal); // Pesquisa as variáveis na direção errada dos escopos, identifica as declarações nos escopos menores para os maiores.
                        ...
                        // Existirem variáveis declaradas no escopo atual que foram anexadas ao escopo global, então remove da lista do escopo global:
                        if (this.listaVariáveisLocal.Count > 0)
                        {
        ...
        {
            string code = $"if({this.Atributos[0].Lexema} {this.OpRelacional.Lexema} {this.Atributos[1].Lexema}) {{";
        ...
        {
            return $"Seleção if: '{this.Atributos[0].Lexema} {this.OpRelacional.Lexema} {this.Atributos[1].Lexema}'";
        }
    }
}
